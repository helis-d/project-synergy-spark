...
function getStoreValue(key) { return readStore()[key]; }
...
function pushRecent(filePath) {
  ...
  s.recentFiles = recents; writeStore(s);
}
...
function buildMenu() {
  ...
  const template = [
    ...(isMac ? [{ role: 'appMenu' }] : []),
    {
      label: 'Dosya',
      submenu: [
        { label: 'Yeni', accelerator: 'CmdOrCtrl+N', click: () => mainWindow.webContents.send('menu-action', 'new') },
        ...
        { label: 'Farklı Kaydet...', accelerator: 'CmdOrCtrl+Shift+S', click: () => handleSaveAs() },
        { type: 'separator' },
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { label: 'Bul', accelerator: 'CmdOrCtrl+F', click: () => mainWindow.webContents.send('menu-action', 'find') }
      ]
    },
    {
      label: 'Görünüm',
    ...
    },
    {
      label: 'Yardım',
      submenu: [
        { label: 'North Hakkında', click: async () => {
...
async function handleSave() {
  ...
  if (currentFilePath) {
    const content = await mainWindow.webContents.executeJavaScript('window.__northGetContent && window.__northGetContent()');
...
ipcMain.handle('open-file', async () => { await handleOpen(); return null; });
...
ipcMain.handle('store-set', async (_e, key, value) => { setStoreValue(key, value); return true; });
...
ipcMain.handle('flow-get-config', async () => {
  const s = readStore();
  return { apiKey: s.flowApiKey || '', model: s.flowModel || 'claude-sonnet-4-6', enabled: s.flowEnabled || false };
});
...
ipcMain.handle('flow-suggest', async (_e, context) => {
  ...
  if (!apiKey) return { error: 'API anahtarı ayarlı değil. Ayarlar > Akış Modu bölümünden ekleyin.' };
  ...
  try {
    ...
    const suggestion = (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('').trim();
    return { suggestion };
  } catch (err) {
    return { error: String(err.message || err) };
  }
});
ipcMain.on('set-dirty', (_e, dirty) => { isDirty = !!dirty; if (mainWindow) {
